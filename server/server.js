...
app.post("/share", function (req, res) {
	getJSON(
		"https://www.alphavantage.co/query?function=TIME_SERIES_INTRADAY&symbol=" +
			req.body.symbol +
			"&interval=1min&apikey=YOUR_API_KEY",
		function (err, data) {
			if (err) throw err;
						...
						dbms.query(bought, [req.body.id, req.body.symbol], function (
							...
							dbms.query(sold, [req.body.id, req.body.symbol], function (
								...
								var val = {
									...
									info: resultname[0][0]["Information"],
...
app.post("/remove", function (req, res) {
	pool.getConnection(function (err, connection) {
		...
		connection.beginTransaction(function (err) {
			...
			connection.query(query, [req.body.id, req.body.symbol], function (
				err,
				result,
				fields
			) {
...
app.post("/save", function (req, res) {
	...
	dbms.query(query, [req.body.username, req.body.id], function (
		...
		dbms.query(currentPasswordQuery, req.body.id, function (err, pass, fields) {
			if (err) throw err;
			bcrypt.compare(req.body.old_password, pass[0][0]["Password"], function (
				err,
				resu
			) {
				if (err) throw err;
